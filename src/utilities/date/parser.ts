import { DATE_SEGMENT_PARTS, type DateSegmentPart, EDITABLE_SEGMENT_PARTS, type Granularity, type HourCycle, type SegmentContentObj, type SegmentPart, type SegmentValueObj, TIME_SEGMENT_PARTS, type TimeSegmentPart, getOptsByGranularity, getPlaceholder, isDateSegmentPart, isSegmentPart } from '../date'
import { isZonedDateTime, toDate } from '@/date'
import type { Formatter } from '../useDateFormatter'
...
export function syncTimeSegmentValues(props: SyncTimeSegmentValuesProps) {
  return Object.fromEntries(TIME_SEGMENT_PARTS.map((part: any) => {
    if (part === 'dayPeriod')
...
export function syncSegmentValues(props: SyncDateSegmentValuesProps) {
  ...
  const dateValues = DATE_SEGMENT_PARTS.map((part: any) => {
    return [part, props.value[part as keyof DateValue]]
  })
...
export function initializeTimeSegmentValues(granularity: 'hour' | 'minute' | 'second'): SegmentValueObj {
  return Object.fromEntries(
    TIME_SEGMENT_PARTS.map((part: any) => {
      if (part === 'dayPeriod')
        return [part, 'AM']
      return [part, null]
    }).filter(([key]: any) => {
...
export function initializeSegmentValues(granularity: Granularity): SegmentValueObj {
  const initialParts = EDITABLE_SEGMENT_PARTS.map((part: any) => {
    if (part === 'dayPeriod')
      return [part, 'AM']
