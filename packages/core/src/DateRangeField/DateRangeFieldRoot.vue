...
import { type Formatter, createContext, useDateFormatter, useDirection, useKbd, useLocale } from '../utilities'
import {
  type DateRange,
  type Granularity,
  type HourCycle,
...
const editableSegmentContents = computed(() => ({ start: segmentContents.value.start.filter(({ part }: any) => part !== 'literal'), end: segmentContents.value.end.filter(({ part }: any) => part !== 'literal') }))
